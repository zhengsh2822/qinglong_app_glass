            ...
            AddSelectAllNavButton(
              ...
              allChecked:
                  ...
                  getListByType(_tabController?.index ?? 0)
                      .where((value) => value.matchSearch(searchText.text))
                      .length,
              color:
...
/// 更新时间比较（早→晚）
int _compareEnvTime(EnvBean a, EnvBean b) => _envTimeMs(a).compareTo(_envTimeMs(b));
...
class _EnvListViewState extends ConsumerState<EnvListView>
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < widget.list.length; i++) {
      ...
      if (value.matchSearch(widget.searchText)) {
        filtered.add(value);
      }
    }
    // 导航栏排序（默认=保持创建时间顺序）
...
class _EnvRecordListViewState extends ConsumerState<EnvRecordListView>
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < widget.list.length; i++) {
      if (widget.list[i].matchSearch(widget.searchText)) {
        filtered.add(widget.list[i]);
      }
    }
    _applyEnvSort(filtered, widget.nameSort, widget.timeSort);
