...
class _GlassSegmentedTabState extends ConsumerState<GlassSegmentedTab> {
  ...
  Widget build(BuildContext context) {
    ...
    // 毛玻璃开关：开启时大胶囊用 BackdropFilter + 半透明底色（alpha 0.45）
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // 大胶囊：毛玻璃开启时半透明底色 + BackdropFilter 模糊；关闭时纯色
        ...
        final Widget capsuleBody = Container(
          ...
          child: ClipRRect(
            ...
            child: Stack(
              ...
              children: [
                // ---------- 滑块（弹簧位置 + 按压缩小 + 加速度果冻变形；每帧 setState 驱动） ----------
                Positioned(
        ...
        // 未开启毛玻璃时用纯透明容器；开启时叠 BackdropFilter 模糊层
        final Widget capsule = widget.blurEnabled
            ? ClipRRect(
                borderRadius: BorderRadius.circular(22),
