...
/// 高光内发光底部弹层容器（与 CapsuleGlowCard 同设计语言）
///
/// 用于底部弹层（ActionSheet / 选择器 / 输入弹窗等）：
/// - 苹果：纯色 #F7F7F7 + 纯白边框 + 浅灰顶部高光 + 纯白内发光
///   + 外阴影（底部弹层阴影向上 offset(0,-4)）
/// - 赛博：深色 cardBg + 青色发光边框(alpha 0.2) + 青色顶部高光
///   + 青色内发光 + 青色外发光
  ...
  Widget build(BuildContext context) {
    ...
    if (isCyber) {
      ...
      glow = [
        ...
        // 底部青色内发光
        BoxShadow(
          color: CyberColors.cyan.withValues(alpha: 0.07),
          blurRadius: 8,
          spreadRadius: 0.4,
        ...
        // 青色外发光
        BoxShadow(
          color: CyberColors.cyan.withValues(alpha: 0.03),
          blurRadius: 14,
          spreadRadius: 0.5,
        ),
      ];
    } else {
      bg = const Color(0xFFF7F7F7); // 未置顶卡片纯色
